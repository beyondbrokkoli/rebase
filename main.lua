...
local function main()
    ...
    -- [PATCH 1: Expand slab to fit both SoA and AoS blocks]
    local FRAME_SOA_COUNT = padded_capacity * 3     -- Uploaded by CPU
    local FRAME_AOS_COUNT = padded_capacity * 4     -- Generated by hash.comp (vec4)
    local FRAME_UINT_COUNT = padded_capacity        -- Sorted Indices
    local FRAME_CELL_COUNT = (NUM_CELLS * 2) + 128;

    local FRAME_TOTAL_WORDS = FRAME_SOA_COUNT + FRAME_AOS_COUNT + FRAME_UINT_COUNT + FRAME_CELL_COUNT
    ...
    local sync_state = renderer.InitSync(vk, device, 10)
    ...
    -- [PATCH 2: Initialize new virtualized pointers]
    pc.soa_upload_idx = 0
    pc.aos_current_idx = 0
    pc.aos_prev_idx = 0
    -- not assigning padded_capacity to pc.struct for some reason
        ...
        if is_resizing then
            if (get_time_hires() - last_resize_time) > RESIZE_COOLDOWN then
                ...
                if new_w[0] > 0 and new_h[0] > 0 then
                    -- 1. HALT THE RENDERER FIRST
                    -- Sever the C-Core's lock on the surface before passing old_swapchain
                    ffi.C.vx_thread_kill()
                    vk.vkDeviceWaitIdle(device)

                    -- 2. Attempt creation on a totally idle driver state
                    local new_sc_state = swapchain_core.Init(vk, vk_state, new_w[0], new_h[0], sc_state.handle)

                    if new_sc_state ~= nil then
                        print("[LUA CO] Window Stable. Initiating Vulkan Rebuild...")

                        -- 3. Destroy old objects ONLY on success
                        graphics.Destroy(vk, vk_state, gfx_state)
                        swapchain_core.Destroy(vk, vk_state, sc_state)
                        renderer.Destroy(vk, device, sync_state, 10)

                        sc_state = new_sc_state
                        gfx_state = graphics.Init(vk, vk_state, new_w[0], new_h[0], desc_state.pipelineLayout, sc_state.format)

                        local fresh_sync = renderer.InitSync(vk, device, 10)
                        ...
                        ffi.C.vx_thread_start() -- 4. Re-ignite the rendering thread

                        print("[LUA CO] Rebuild Complete.")
                        is_resizing = false
                        last_time = get_time_hires()
                    else
                        -- 5. FAILURE: Surface is volatile.
                        -- Thread is dead. Do not restart it. Let it rest and retry next frame.
            ...
            if write_idx ~= -1 then
                -- [PATCH 3: Virtualize pointers to enable lazy t-1 access]
                local RING_SIZE = 4 -- Matches main.c RenderRing size
                local prev_idx = (write_idx - 1 + RING_SIZE) % RING_SIZE

                local current_frame_offset = write_idx * FRAME_TOTAL_WORDS
                local prev_frame_offset = prev_idx * FRAME_TOTAL_WORDS

                -- 1. CPU streams SoA directly into the start of the frame's block
                ...
                -- 2. Define the offset for the GPU-generated AoS block (right after SoA)
                local aos_local_offset = padded_capacity * 3

                -- 3. Route the shaders to the correct slabs
                ...
                -- 4. Shift spatial grid data past both the SoA and AoS blocks
                ...
                if active_render_mode == MODE_DUAL then
                    -- Cmd0 (Geometry): Scan the whole grid, shader keeps EVENS
                    cmd0.first_instance = 0
                    cmd0.instance_count = pc.particle_count

                    -- Cmd1 (Points): Scan the whole grid, shader keeps ODDS
