...
function GraphicsPipeline.Init(vk, core_state, width, height, pipelineLayout, colorFormat)
    ...
    dViewInfo.viewType = 1
    dViewInfo.format = 126
    dViewInfo.subresourceRange.aspectMask = 2
    ...
    local vertInfo = ffi.new("VkShaderModuleCreateInfo", { 
        sType = 16, 
        codeSize = string.len(vertCode), 
        pCode = ffi.cast("const uint32_t*", vertCode) 
    })
    local fragInfo = ffi.new("VkShaderModuleCreateInfo", { 
        sType = 16, 
        codeSize = string.len(fragCode), 
        pCode = ffi.cast("const uint32_t*", fragCode) 
    })
